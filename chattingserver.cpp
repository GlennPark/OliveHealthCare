...
ChattingServer::ChattingServer(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ChattingServer), totalSize(0), byteReceived(0)
{
    ui->setupUi(this);
    QList<int> sizes;
    sizes << 120 << 500;
    ...
    qDebug()<<"port"<<chatServer->serverPort();
...
void ChattingServer::receiveData( )
{
    ...
    QString ip = clientConnection->peerAddress().toString();    //ip 를 문자열로 변환
...
void ChattingServer::removeClient()
{
    QTcpSocket *clientConnection = dynamic_cast<QTcpSocket *>(sender( ));
    if(clientConnection == nullptr)     //예외처리 해 줄것
    {
...
void ChattingServer::addClient(int id, QString name)
{
    clientIDList << id;
...
void ChattingServer::on_clientTreeWidget_customContextMenuRequested(const QPoint &pos)
{
...
void ChattingServer::inviteClient()
{
...
void ChattingServer::acceptConnection()
{
...
void ChattingServer::readClient()
{
